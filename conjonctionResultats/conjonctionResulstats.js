...
function determineMessage(totalScoreBef, totalScoreAft) {
    if (totalScoreBef <= 2 && totalScoreAft >= 6) {
    ...
    if (totalScoreBef >= 6 && totalScoreAft <= 2) {
    ...
    if (totalScoreBef >= 6 && totalScoreAft >= 6) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    //display score
    const beforeScores = ["G3Q1NConfBef", "G3Q2NconfBef", "G3Q3confBef", "G3Q4ConfBef"];
    const afterScores = ["G3Q5NconfAft", "G3Q6NconfAft", "G3Q7ConfAft", "G3Q8ConfAft"];
    ...
    const conjonctionJeu = sessionStorage.getItem('conjonctionJeu');
...
function calculateScore(...values) {
  ...
  const scoreConfBef = calculateScore(
    sessionStorage.getItem('G3Q3confBef'),
    sessionStorage.getItem('G3Q4ConfBef')
  );
  
  const scoreNconfBef = calculateScore(
    sessionStorage.getItem('G3Q1NConfBef'),
    sessionStorage.getItem('G3Q2NconfBef')
  );
  
  const scoreConfAft = calculateScore(
    sessionStorage.getItem('G3Q7ConfAft'),
    sessionStorage.getItem('G3Q8ConfAft')
  );
  
  const scoreNconfAft = calculateScore(
    sessionStorage.getItem('G3Q5NconfAft'),
    sessionStorage.getItem('G3Q6NconfAft')
  );
  

//Time spent on the task
const timeScores = ["TimeG3Q1ConfBef", "TimeG3Q2NconfBef", "TimeG3Q3NconfBef", "TimeG3Q4ConfBef", 
    "TimeG3Q5NconfAft", "TimeG3Q6NconfAft", "TimeG3Q7ConfAft", "TimeG3Q8ConfAft"];
...
//Task start time
const conjonctionStartTime = sessionStorage.getItem('conjonctionStartTime');

function sendData(uid, conjonctionStartTime, conjonctionJeu, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent) {

    let data = {
        "cr40e_cogiuserid": uid,
        "cr40e_tempsdebuttache": conjonctionStartTime,
        "cr40e_cogitaskname": conjonctionJeu,
...
if (sessionStorage.getItem("age") !== null) {
  ...
  sendData(uid, conjonctionStartTime, conjonctionJeu, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent);
...
function init() {
    ...
    displayResult("result", "Score total: " + totalScore + " / 16");
    displayResult("resultBef", totalScoreBef + " / 8");
    displayResult("resultAft", totalScoreAft + " / 8");
...
function calculateScore(...values) {
    const sum = values.reduce((acc, val) => acc + Number(val), 0);
    return sum === 1 ? 25 : sum === 2 ? 50 : sum === 3 ? 75 : sum === 4 ? 100 : sum;
  }
...
sendData(uid, conjonctionStartTime, conjonctionJeu, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent);
    ...
    document.getElementById('home_button').addEventListener('click', async function(event) {
        ...
        try {
            ...
            await sendData(uid, conjonctionStartTime, conjonctionJeu, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent);
