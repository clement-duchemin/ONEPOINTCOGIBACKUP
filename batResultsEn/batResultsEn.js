...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const batAndBallGame = sessionStorage.getItem('batAndBallGame');
...
function calculateScore(...values) {
  ...
  const scoreConfBef = calculateScore(
    sessionStorage.getItem('G1Q1ConfBefEn'),
    sessionStorage.getItem('G1Q4ConfBefEn')
  );
  
  const scoreNconfBef = calculateScore(
    sessionStorage.getItem('G1Q2NconfBefEn'),
    sessionStorage.getItem('G1Q3NconfBefEn')
  );
  
  const scoreConfAft = calculateScore(
    sessionStorage.getItem('G1Q7ConfAftEn'),
    sessionStorage.getItem('G1Q8ConfAftEn')
  );
  
  const scoreNconfAft = calculateScore(
    sessionStorage.getItem('G1Q5NconfAftEn'),
    sessionStorage.getItem('G1Q6NconfAftEn')
  );
  

//Time spent on the task
const timeScores = ["TimeG1Q1ConfBefEn", "TimeG1Q2NconfBefEn", "TimeG1Q3NconfBefEn", "TimeG1Q4ConfBefEn", 
    "TimeG1Q5NconfAftEn", "TimeG1Q6NconfAftEn", "TimeG1Q7ConfAftEn", "TimeG1Q8ConfAftEn"];
...
//Task start time
const batAndBallStartTime = sessionStorage.getItem('batAndBallStartTime');

function sendData(uid, batAndBallStartTime, batAndBallGame, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent) {

    let data = {
        "cr40e_cogiuserid": uid,
        "cr40e_tempsdebuttache": batAndBallStartTime,
...
if (sessionStorage.getItem("age") !== null) {
  ...
  sendData(uid, batAndBallStartTime, batAndBallGame, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent);
...
//main page test patch full functions
function sendData(uid, batteEtBalleStart, batAndBallGame, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, consent) {
    ...
    webapi.safeAjax({
        ...
        success: function(res, status, xhr) { // Moved inside the object
            console.log("entityID: " + xhr.getResponseHeader("entityid"));
            entityID = xhr.getResponseHeader("entityid"); // Store entityID globally
...
//Task start time
const batteEtBalleStart = sessionStorage.getItem('batAndBallStartTime');

sendData(uid, batteEtBalleStart, batAndBallGame, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, consent);
    // Ensure the DOM is fully loaded before setting up event listeners
    document.getElementById('home_button').addEventListener('click', async function(event) {
        ...
        try {
            ...
            await sendData(uid, batteEtBalleStart, batAndBallGame, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent);
...
function attachFormSubmitListener(uid, batteEtBalleStart, batAndBallGame, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, consent) {
    document.getElementById('replay_button').addEventListener('click', function(e) {
        ...
        // Assuming you want to update certain fields based on the form's input
        let updateData = {
                ...
                "cr40e_cogiconsent": consent.toString(),
                "cr40e_cogiage": age,
                "cr40e_cogisexe": gender.toString(),
                "cr40e_cogieducation": education.toString(),
                "cr40e_cogipays": country.toString(),
                //"cr40e_cogiemail": email.toString()
        };

        webapi.safeAjax({
            type: "PUT",
            url: "/_api/cr40e_cogitumtabs/" + entityID,
            contentType: "application/json",
...
attachFormSubmitListener(uid, batteEtBalleStart, batAndBallGame, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, consent);
    ...
    document.getElementById('keepPlaying_button').addEventListener('click', async function(event) {
        event.preventDefault(); // Stop the default navigation
                window.location.href = "/batRetestQuestions";
...
//main page
/* code that works but need to be adjusted for data sending
...
sendData(uid, batteEtBalleStart, batAndBallGame, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent);
    ...
    document.getElementById('replay_button').addEventListener('click', async function(event) {
        ...
        try {
            ...
            await sendData(uid, batteEtBalleStart, batAndBallGame, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent);
            // Navigate to the homepage after successful data sending
            window.location.href = "/batQuestionsEn";
        } catch (error) {
            console.error("Error sending data:", error);
            // Handle the error accordingly. You might want to inform the user or log the error.
        }
...
//test patch simplified function

function sendData(uid) {
    let data = {
        "cr40e_cogiuserid": uid,
        "cr40e_cogitaskname": "yo"
    };
...
function attachFormSubmitListener(uid) {
    document.getElementById('replay_button').addEventListener('click', function(e) {
        e.preventDefault(); // Prevent default form submission
