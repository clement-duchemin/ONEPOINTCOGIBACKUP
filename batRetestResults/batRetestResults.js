...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const batAndBallRetestGame = sessionStorage.getItem('batAndBallRetestGame');
...
function calculateScore(...values) {
  ...
  const scoreConfBef = calculateScore(
    sessionStorage.getItem('G1Q9ConfBefEn'),
  );
  
  const scoreConfAft = calculateScore(
    sessionStorage.getItem('G1Q12ConfAftEn'),
    sessionStorage.getItem('G1Q13ConfAftEn')
  );
  
  const scoreNconfAft = calculateScore(
    sessionStorage.getItem('G1Q10NconfAftEn'),
    sessionStorage.getItem('G1Q11NconfAftEn')
  );
  

//Time spent on the task
const timeScores = ["TimeG1Q9ConfBefEn", "TimeG1Q12ConfAftEn", "TimeG1Q13ConfAftEn", "TimeG1Q10NconfAftEn", 
    "TimeG1Q11NconfAftEn"];
...
//Task start time
const batAndBallRetestStartTime = sessionStorage.getItem('batAndBallRetestStartTime');

function sendData(uid, batAndBallRetestStartTime, batAndBallRetestGame, scoreConfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent) {

    let data = {
        "cr40e_cogiuserid": uid,
        "cr40e_tempsdebuttache": batAndBallRetestStartTime,
        "cr40e_cogitaskname": batAndBallRetestGame,
...
if (sessionStorage.getItem("age") !== null) {
  ...
  sendData(uid, batAndBallRetestStartTime, batAndBallRetestGame, scoreConfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent);
  ...
  // Event listener for the modal close icon
    ...
    document.getElementById('replay_button-en').addEventListener('click',  function() {
        ...
        window.location.href = "https://cogitum.powerappsportals.com/batRetestQuestions/";
    });
