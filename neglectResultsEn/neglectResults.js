...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const neglectGame = sessionStorage.getItem('neglectGame');
...
function calculateScore(...values) {
  ...
  const scoreConfBef = calculateScore(
    sessionStorage.getItem('G2Q3confBefEn'),
    sessionStorage.getItem('G2Q4ConfBefEn')
  );
  
  const scoreNconfBef = calculateScore(
    sessionStorage.getItem('G2Q1NConfBefEn'),
    sessionStorage.getItem('G2Q2NconfBefEn')
  );
  
  const scoreConfAft = calculateScore(
    sessionStorage.getItem('G2Q7ConfAftEn'),
    sessionStorage.getItem('G2Q8ConfAftEn')
  );
  
  const scoreNconfAft = calculateScore(
    sessionStorage.getItem('G2Q5NconfAftEn'),
    sessionStorage.getItem('G2Q6NconfAftEn')
  );
  

//Time spent on the task
const timeScores = ["TimeG2Q1ConfBefEn", "TimeG2Q2NconfBefEn", "TimeG2Q3NconfBefEn", "TimeG2Q4ConfBefEn", 
    "TimeG2Q5NconfAftEn", "TimeG2Q6NconfAftEn", "TimeG2Q7ConfAftEn", "TimeG2Q8ConfAftEn"];
...
//Task start time
const neglectStartTime = sessionStorage.getItem('neglectStartTime');

function sendData(uid, neglectStartTime, neglectGame, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent) {

    let data = {
        "cr40e_cogiuserid": uid,
        "cr40e_tempsdebuttache": neglectStartTime,
        "cr40e_cogitaskname": neglectGame,
...
if (sessionStorage.getItem("age") !== null) {
  ...
  sendData(uid, neglectStartTime, neglectGame, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent);
...
document.addEventListener('DOMContentLoaded', function() {
    const uid = sessionStorage.getItem("userUID");
    const negligenceGame = sessionStorage.getItem('neglectGame');
    const age = Number(sessionStorage.getItem("age"));
    const gender = sessionStorage.getItem("sex");
    const education = sessionStorage.getItem("education");
    const country = sessionStorage.getItem("pays");
...
//Task start time
const negligenceStart = sessionStorage.getItem('neglectStartTime');
...
sendData(uid, negligenceStart, negligenceGame, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent);
    ...
    document.getElementById('replay_button').addEventListener('click', async function(event) {
        ...
        try {
            ...
            await sendData(uid, negligenceStart, negligenceGame, scoreConfBef, scoreNconfBef, scoreConfAft, scoreNconfAft, time, age, gender, education, country, email, consent);
            // Navigate to the homepage after successful data sending
            window.location.href = "/neglectQuestionsEn";
        } catch (error) {
            console.error("Error sending data:", error);
            // Handle the error accordingly. You might want to inform the user or log the error.
        }
    ...
    document.getElementById('keepPlaying_button').addEventListener('click', async function(event) {
        event.preventDefault(); // Stop the default navigation
                window.location.href = "/neglectRetestQuestions";
